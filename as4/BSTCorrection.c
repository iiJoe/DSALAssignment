/**
 * BSTCorrection.c -- Q2: Correction of Binary Search Tree
 *
 * Written on Tuesday, 22 February 2022.
 */
#include <stdio.h>
#include <stdlib.h>
...
void traversal(BTNode* curr, Stack* sPtr) {
  if (curr) {
    traversal(curr->right, sPtr);
    push(sPtr, curr);
    traversal(curr->left, sPtr);
  }
}
...
void BSTCorrection(BTNode* root)
{
  ...
  // Inorder traversal, pop element in stack one by one
  traversal(root, &elements);
  BTNode* firstErr = NULL, *secondErr = NULL, *curr, *tmp = NULL;

  while (!isEmptyStack(elements)) {
    curr = peek(elements);
    if (tmp) {
  ...
  int temp = firstErr->item;
  firstErr->item = secondErr->item;
  secondErr->item = temp;
}
