/**
 * q7.c -- Q7: Shortest Distances
 *
 * Written on Saturday, 26 March 2022.
 */
...
int SD(Graph g, int v, int z){
  ...
  int curr, explored[g.V];
  for (int j = 0; j < g.V; j++) explored[j] = 0;

  while (!isEmptyQueue(q) && getFront(q) != z - 1) {
    curr = getFront(q);
    for (int i = 0; i < g.V; i++) {
      if (i != v - 1 && g.matrix[curr][i] == 1 && explored[i] == 0) {
        enqueue(&q, i);
        explored[i] = explored[curr] + 1;
      }
    }
    dequeue(&q);
  }
