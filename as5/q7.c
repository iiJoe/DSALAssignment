/**
 * q7.c -- Q7: Graph Representation: Adjacency List to Adjacency Matrix
 *
 * Written on Sunday, 20 March 2022.
 */
...
void adjL2adjM(Graph *g){
  ...
  g->type = ADJ_MATRIX;
  ListNode **adjList = g->adj.list;

  for (int i = 0; i < g->V; i++) {
    ListNode *curr = adjList[i];
    while (curr) {
      g->adj.matrix[i][curr->vertex - 1] = 1;
      curr = curr->next;
    }
  }
